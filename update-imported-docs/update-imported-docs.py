...
def process_file(src, dst, repo_path, repo_dir, root_dir, gen_absolute_links):
    ...
    for src in glob.glob(pattern):
        ...
        if not os.path.isfile(src):
            print(f"[Error] skipping non-regular path {src}")
            continue
        ...
        try:
            print(f"Writing doc: {dst}")
            with open(dst, "w") as dstFile:
                if gen_absolute_links:
                    src_dir = os.path.dirname(src)
                    remote_prefix = f"{repo_path}/tree/master"
            ...
            print(f"[Error] failed in writing target file {dst}: {ex}")
            continue
...
def main():
    ...
    print(f"k8s_release is {k8s_release}")

    # if release string does not contain patch num, add zero
    if len(k8s_release) == 4:
        k8s_release = f"{k8s_release}.0"
        print(f"k8s_release updated to {k8s_release}")
    ...
    try:
        ...
        print(f"[Error] failed in loading config file - {str(ex)}")
        return -2
    ...
    try:
        ...
        print(f"[Error] Unable to create temp work_dir {work_dir}; error: {ose}")
        return -2
    ...
    for repo in config_data["repos"]:
        ...
        if "remote" not in repo:
            print(f"[Error] repo {repo_name} missing repo path")
            continue
        ...
        if not matches:
            print(f"[Error] repo path for {repo_name} is invalid")
            continue

        repo_path = os.path.join("src", matches['prefix'])
        ...
        if res != 0:
            print(f"[Error] failed in cloning repo {repo_name}")
            continue
        ...
        if "generate-command" in repo:
                                ...
                                (
                                    (
                                        (
                                            (
                                                (
                                                    f"export K8S_RELEASE={k8s_release}"
                                                    + "\n"
                                                    + "export GOPATH="
                                                )
                                                + work_dir
            ...
            print(f"Generating docs for {repo_name} with {gen_cmd}")
            res = subprocess.call(gen_cmd, shell=True)
            if res != 0:
                print(f"[Error] failed in generating docs for {repo_name}")
                continue
    ...
    print(
        f"Completed docs update. Now run the following command to commit:\n\n git add .\n git commit -m <comment>\n git push\n delete temp dir {work_dir} when done "
    )
